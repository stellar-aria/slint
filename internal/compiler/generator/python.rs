...
impl TryFrom<&Rc<crate::langtype::Struct>> for PyStruct {
    ...
    fn try_from(structty: &Rc<crate::langtype::Struct>) -> Result<Self, Self::Error> {
        let Some(name) = &structty.name else {
...
pub fn generate_py_module(
        ...
        match &export.1 {
            ...
            Either::Right(ty) => match &ty {
                Type::Struct(s) if s.name.is_some() && s.node.is_some() => {
                    let orig_name = s.name.as_ref().unwrap().clone();
                    if export.0.name != orig_name {
                        struct_aliases.entry(orig_name).or_default().push(export.0.name.clone());
                    }
                }
                Type::Enumeration(en) => {
    ...
    for ty in &doc.used_types.borrow().structs_and_enums {
        match ty {
            Type::Struct(s) => {
                module.structs_and_enums.extend(PyStruct::try_from(s).ok().map(|mut pystruct| {
                    pystruct.aliases = struct_aliases
                        .remove(s.name.as_ref().expect(
                            "Struct must have a name, otherwise PyStruct::try_from would've failed",
                        ))
                        .unwrap_or_default();
...
pub fn generate(
    ...
    {
        let expected_api_str = serde_json::to_string(&pymodule).unwrap();

        file.imports.push(SmolStr::new_static("os"));
        file.trailing_code.push(format_smolstr!(
            "globals().update(vars(slint._load_file_checked(path=os.path.join(os.path.dirname(__file__), '{}'), expected_api=r'{}')))",
