        ...
        window_inner
            .draw_contents(|components| -> Result<(), PlatformError> {
                ...
                let initial_transform = if rotation_degrees != 0.0 {
                    vello::kurbo::Affine::translate(translation)
                ...
                // Render window item background if not a solid color (already cleared above)
                if let Some(window_item_rc) = window_inner.window_item_rc() {
                    let window_item =
                        window_item_rc.downcast::<i_slint_core::items::WindowItem>().unwrap();
                    if !matches!(window_item.as_pin_ref().background(), Brush::SolidColor(..)) {
                ...
                if let Some(collector) = &self.rendering_metrics_collector.borrow().as_ref() {
                    // Vello does not currently expose per-frame draw call counts or similar
                    // statistics, so metrics remain at their default (empty) values.
