...
pub struct VelloItemRenderer<'a> {
    ...
    wgpu_texture_reader: Option<&'a dyn Fn(&wgpu_28::Texture) -> Option<(u32, u32, Vec<u8>)>>,
...
impl<'a> VelloItemRenderer<'a> {
    ...
    pub fn set_wgpu_texture_reader(
        &mut self,
        reader: &'a dyn Fn(&wgpu_28::Texture) -> Option<(u32, u32, Vec<u8>)>,
    ) {
...
impl<'a> ItemRenderer for VelloItemRenderer<'a> {
    ...
    fn draw_border_rectangle(
        ...
        // Convert radii to physical pixels and clamp each corner to half the smallest dimension
        let half_min = (self.to_physical(rect_geom.size.width) / 2.)
            .min(self.to_physical(rect_geom.size.height) / 2.);
        let clamp = |r: f32| (self.to_physical(r)).min(half_min);
        let tl = clamp(border_radius_logical.top_left);
        let tr = clamp(border_radius_logical.top_right);
        let br = clamp(border_radius_logical.bottom_right);
        let bl = clamp(border_radius_logical.bottom_left);
        let phys_border = self.to_physical(border_width.get());

        // Build per-corner RoundedRectRadii, clamping any negative offsets to zero.
        let make_radii = |offset: f32| {
            peniko::kurbo::RoundedRectRadii::new(
                (tl + offset).max(0.) as f64,
                (tr + offset).max(0.) as f64,
                (br + offset).max(0.) as f64,
                (bl + offset).max(0.) as f64,
            )
        };

        // The border stroke is always centered at border_width/2 from the outer edge, covering
        // exactly the region [0, border_width] regardless of whether the border is opaque.
        let maybe_border_shape = if border_color.is_transparent() || border_width.get() <= 0. {
            None
        } else {
            let border_center_rect = LogicalRect::new(
                LogicalPoint::new(border_width.get() / 2., border_width.get() / 2.),
                LogicalSize::new(
                    rect_geom.size.width - border_width.get(),
                    rect_geom.size.height - border_width.get(),
                ),
            );
            let stroke_radii = make_radii(-phys_border / 2.);
            Some(RoundedRect::from_rect(self.to_kurbo_rect(border_center_rect), stroke_radii))
        };

        // For an opaque border the background fill is inset to avoid overdraw under the border;
        // for a transparent/semi-transparent border the background shows through, so fill the full rect.
        let background_shape = if opaque_border {
            let inner_rect = LogicalRect::new(
            ...
            RoundedRect::from_rect(self.to_kurbo_rect(inner_rect), make_radii(-phys_border))
        } else {
            RoundedRect::from_rect(self.to_kurbo_rect(rect_geom), make_radii(0.))
        };
        ...
        if let Some(border_shape) = &maybe_border_shape {
            let border_bounds = border_shape.rect();
    ...
    fn draw_image(
        ...
        use i_slint_core::graphics::{IntRect, fit};
        ...
        match image_inner {
            ImageInner::EmbeddedImage { buffer, .. } => {
                ...
                // Clip drawing to the intended region (alpha is already applied via the sampler).
                // push_clip_layer's transform only moves the clip shape into scene space.
                ...
                // Draw the image with its full positioning transform.
            ...
            ImageInner::NineSlice(nine_slice) => {
                ...
                if let ImageInner::EmbeddedImage { buffer, .. } = inner_image_inner {
                    ...
                    for fit in fits {
                        ...
                        // Clip to target rect (in local physical space, positioned by current_transform).
                        // The layer transform only moves the clip shape — it does NOT affect draw calls.
                        self.scene.push_clip_layer(
                            peniko::Fill::NonZero,
                            self.current_transform(),
                            &clip_rect,
                        ...
                        // Draw the full image with slice_transform, which correctly maps the source
                        // clip_rect region of the image into the target rect on screen.
                        let full_image_rect =
            ...
            ImageInner::WGPUTexture(wgpu_texture) => {
                ...
                // Handle WGPU 28 textures
                #[allow(irrefutable_let_patterns)]
                if let WGPUTexture::WGPU28Texture(texture) = wgpu_texture {
    ...
    fn draw_text(
        ...
        sharedparley::draw_text(self, text, Some(item_rc), size, None);
    }
    ...
    fn draw_text_input(
        ...
        sharedparley::draw_text_input(self, text_input, item_rc, size, None);
    }
    ...
    fn combine_clip(
        ...
        let half_min =
            (self.to_physical(rect.size.width) / 2.).min(self.to_physical(rect.size.height) / 2.);
        let clamp = |r: f32| (self.to_physical(r)).min(half_min) as f64;
        let has_radius = radius.top_left > 0.
            || radius.top_right > 0.
        ...
        // Apply actual clipping using Vello's clip-layer API.
        // push_clip_layer is a pure clip (no compositing group overhead).
        // Note: in Vello the transform argument to push_clip_layer only moves the clip shape;
        // it does NOT change the coordinate system for subsequent draw calls.
        if has_radius {
            let clip_shape = RoundedRect::from_rect(self.to_kurbo_rect(rect), radii);
            self.scene.push_clip_layer(peniko::Fill::NonZero, current_transform, &clip_shape);
        } else {
    ...
    fn draw_cached_pixmap(
        ...
        if let Some((width, height, data)) = image_data_opt {
            ...
            // The pixmap data is already in physical pixels (the caller scales by dpr),
            // so draw it at its native pixel size — do NOT call to_physical() here.
            let target_rect = peniko::kurbo::Rect::new(0.0, 0.0, width as f64, height as f64);
...
impl<'a> GlyphRenderer for VelloItemRenderer<'a> {
    ...
    fn platform_brush_for_color(&mut self, color: &Color) -> Option<Self::PlatformBrush> {
        if color.alpha() == 0 { None } else { Some(VelloBrush::Fill(to_peniko_color(color))) }
    }
